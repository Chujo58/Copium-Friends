import { useEffect, useMemo, useState } from "react";
import { ArrowLeft, Moon, RefreshCw, Sun } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
import { createQuiz, listQuizzes } from "../lib/api";
import { getStoredUsername } from "../lib/identity";
import { useTheme } from "../theme-context.jsx";

const QUIZ_TYPES = [
  { value: "mcq", label: "QCM (Multiple Choice)" },
  { value: "short", label: "Short Answer" },
  { value: "long", label: "Long Answer" },
];

function formatQuizType(value) {
  const normalized = String(value || "").toLowerCase();
  if (normalized === "mcq") return "QCM";
  if (normalized === "short") return "Short";
  if (normalized === "long") return "Long";
  return "Quiz";
}

function formatDate(timestamp) {
  if (!Number.isFinite(timestamp)) return "-";
  return new Date(timestamp).toLocaleDateString();
}

export default function Quizzes() {
  const navigate = useNavigate();
  const location = useLocation();
  const { isDay, toggleMode } = useTheme();

  const username =
    location.state?.username?.trim() || getStoredUsername() || "Guest";
  const serverName =
    location.state?.serverName?.trim() ||
    sessionStorage.getItem("activeServerName") ||
    "My Server";
  const selectedCat =
    location.state?.selectedCat ||
    sessionStorage.getItem("activeSelectedCat") ||
    sessionStorage.getItem("selectedCatId") ||
    "";
  const selectedAction =
    location.state?.selectedAction ||
    sessionStorage.getItem("activeSelectedAction") ||
    "";

  const [quizTitle, setQuizTitle] = useState("");
  const [quizPrompt, setQuizPrompt] = useState("");
  const [quizType, setQuizType] = useState("mcq");
  const [questionCount, setQuestionCount] = useState("10");
  const [searchText, setSearchText] = useState("");
  const [quizzes, setQuizzes] = useState([]);
  const [loadingQuizzes, setLoadingQuizzes] = useState(true);
  const [isCreating, setIsCreating] = useState(false);
  const [error, setError] = useState("");

  const filteredQuizzes = useMemo(() => {
    const needle = searchText.trim().toLowerCase();
    if (!needle) return quizzes;
    return quizzes.filter((quiz) => {
      const text = [quiz.title, quiz.prompt, quiz.type].join(" ").toLowerCase();
      return text.includes(needle);
    });
  }, [quizzes, searchText]);

  function normalizedQuestionCount() {
    const numeric = Number(questionCount);
    if (!Number.isFinite(numeric)) return 10;
    return Math.max(1, Math.min(50, Math.floor(numeric)));
  }

  async function loadQuizzes(showLoading = true) {
    if (showLoading) setLoadingQuizzes(true);
    setError("");
    try {
      const payload = await listQuizzes(username);
      setQuizzes(Array.isArray(payload.quizzes) ? payload.quizzes : []);
    } catch (requestError) {
      setError(requestError.message || "Could not load quizzes");
    } finally {
      if (showLoading) setLoadingQuizzes(false);
    }
  }

  useEffect(() => {
    loadQuizzes(true);
  }, []);

  function openQuiz(quizId) {
    if (!quizId) return;
    navigate(`/quiz/${quizId}`, {
      state: {
        from: location.state?.from || "quizzes",
        username,
        serverName,
        serverId: location.state?.serverId || "",
        serverCode: location.state?.serverCode || "",
        memberId: location.state?.memberId || "",
        selectedCat,
        selectedAction,
      },
    });
  }

  async function handleCreateQuiz() {
    if (!quizPrompt.trim() || isCreating) return;
    setError("");
    setIsCreating(true);
    try {
      const payload = await createQuiz({
        username,
        serverName,
        title: quizTitle.trim(),
        prompt: quizPrompt.trim(),
        type: quizType,
        questionCount: normalizedQuestionCount(),
      });
      const nextQuiz = payload?.quiz;
      if (nextQuiz) {
        setQuizzes((prev) => [nextQuiz, ...prev]);
        openQuiz(nextQuiz.id);
      } else {
        await loadQuizzes(false);
      }
      setQuizTitle("");
      setQuizPrompt("");
    } catch (requestError) {
      setError(requestError.message || "Could not create quiz");
    } finally {
      setIsCreating(false);
    }
  }

  function handleBack() {
    if (location.state?.from === "session") {
      navigate("/session", {
        state: {
          serverId: location.state?.serverId || "",
          serverName,
          serverCode: location.state?.serverCode || "",
          memberId: location.state?.memberId || "",
          selectedCat,
          selectedAction,
        },
      });
      return;
    }
    navigate("/dashboard", { state: { username } });
  }

  return (
    <div
      className={`relative min-h-screen overflow-hidden px-4 py-8 transition-all duration-[2000ms] md:px-8 ${
        isDay
          ? "bg-gradient-to-t from-[#88A7BE] via-[#A6C0D2] to-[#C8D8E3]"
          : "bg-gradient-to-t from-[#0F172A] via-[#1E293B] to-[#334155]"
      }`}
    >
      <button
        onClick={toggleMode}
        aria-label="Toggle light/dark mode"
        className={`group absolute right-12 top-12 z-20 flex h-16 w-16 items-center justify-center rounded-full transition-all duration-[2000ms] ${
          isDay
            ? "bg-yellow-100 shadow-[0_0_80px_rgba(253,224,71,0.4)] hover:shadow-[0_0_100px_rgba(253,224,71,0.6)]"
            : "bg-slate-100 shadow-[0_0_50px_rgba(255,255,255,0.2)] hover:shadow-[0_0_70px_rgba(255,255,255,0.4)]"
        }`}
      >
        <div className="transition-transform duration-700 group-hover:rotate-12 group-active:scale-90">
          {isDay ? (
            <Sun className="h-8 w-8 text-yellow-400/60" />
          ) : (
            <Moon className="h-7 w-7 fill-slate-400/10 text-slate-400/60" />
          )}
        </div>
      </button>

      <div className="absolute -top-24 -left-20 h-80 w-80 rounded-full bg-white/30 blur-2xl" />
      <div className="absolute -bottom-24 -right-16 h-80 w-80 rounded-full bg-primary/25 blur-2xl" />

      <div className="relative mx-auto max-w-6xl rounded-[2.5rem] border-4 border-primary/40 bg-surface/40 p-5 shadow-2xl backdrop-blur-xl md:p-8">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <button
            onClick={handleBack}
            className="inline-flex h-12 items-center gap-2 rounded-xl border-2 border-primary/45 bg-primary px-4 font-card text-xl font-black tracking-tight text-white transition hover:bg-accent"
          >
            <ArrowLeft className="h-5 w-5" />
            Back
          </button>
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-primary/35 bg-white/80 px-3 py-1 text-sm font-bold text-slate-800">
              {username}
            </span>
            <span className="rounded-full border border-primary/35 bg-white/80 px-3 py-1 text-sm font-bold text-slate-800">
              {serverName}
            </span>
          </div>
        </div>

        <section className="mb-5 rounded-[1.7rem] border-2 border-primary/35 bg-white/70 p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="font-card text-5xl font-black tracking-tight text-slate-900">
                Quizzes
              </h1>
            </div>
            <div className="text-right text-sm font-semibold text-slate-700">
              <p>{quizzes.length} quizzes</p>
              <p>
                {quizzes.reduce(
                  (sum, quiz) => sum + Number(quiz.questionsCount || 0),
                  0,
                )}{" "}
                questions total
              </p>
            </div>
          </div>
        </section>

        <section className="grid gap-5 lg:grid-cols-2">
          <div className="rounded-[1.7rem] border-2 border-primary/35 bg-white/80 p-5">
            <h2 className="font-card text-4xl font-black tracking-tight text-slate-900">
              Create A Quiz
            </h2>
            <input
              value={quizTitle}
              onChange={(event) => setQuizTitle(event.target.value)}
              placeholder="Quiz name (optional)"
              maxLength={80}
              className="mt-4 h-12 w-full rounded-xl border-2 border-primary/35 bg-white/95 px-3 text-lg font-semibold text-slate-800 outline-none"
            />
            <textarea
              value={quizPrompt}
              onChange={(event) => setQuizPrompt(event.target.value)}
              placeholder="Describe what to quiz: chapter, topic, exam focus..."
              rows={6}
              maxLength={1200}
              className="mt-3 w-full rounded-xl border-2 border-primary/35 bg-white/95 p-3 text-lg font-semibold text-slate-800 outline-none"
            />

            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="text-sm font-semibold text-slate-700">
                Type
                <select
                  value={quizType}
                  onChange={(event) => setQuizType(event.target.value)}
                  className="mt-1 h-11 w-full rounded-xl border-2 border-primary/35 bg-white/95 px-3 text-base font-semibold text-slate-800 outline-none"
                >
                  {QUIZ_TYPES.map((typeOption) => (
                    <option key={typeOption.value} value={typeOption.value}>
                      {typeOption.label}
                    </option>
                  ))}
                </select>
              </label>

              <label className="text-sm font-semibold text-slate-700">
                Number of questions
                <input
                  type="number"
                  min="1"
                  max="50"
                  value={questionCount}
                  onChange={(event) => setQuestionCount(event.target.value)}
                  className="mt-1 h-11 w-full rounded-xl border-2 border-primary/35 bg-white/95 px-3 text-base font-bold text-slate-800 outline-none"
                />
              </label>
            </div>

            <div className="mt-3 flex gap-2">
              <button
                onClick={handleCreateQuiz}
                disabled={!quizPrompt.trim() || isCreating}
                className={`h-12 rounded-xl border-2 px-6 font-card text-xl font-black tracking-tight transition ${
                  quizPrompt.trim() && !isCreating
                    ? "border-primary/45 bg-primary text-white hover:bg-accent"
                    : "cursor-not-allowed border-slate-300 bg-slate-200 text-slate-500"
                }`}
              >
                {isCreating ? "Creating..." : "Create Quiz"}
              </button>
              <button
                onClick={() => {
                  setQuizTitle("");
                  setQuizPrompt("");
                  setQuizType("mcq");
                  setQuestionCount("10");
                }}
                className="h-12 rounded-xl border-2 border-primary/35 bg-white px-6 font-card text-xl font-black tracking-tight text-slate-800 transition hover:bg-slate-100"
              >
                Clear
              </button>
            </div>
          </div>

          <div className="rounded-[1.7rem] border-2 border-primary/35 bg-white/80 p-5">
            <div className="flex items-center justify-between gap-3">
              <h2 className="font-card text-4xl font-black tracking-tight text-slate-900">
                Old Quizzes
              </h2>
              <button
                onClick={() => loadQuizzes(false)}
                className="inline-flex h-11 items-center gap-2 rounded-xl border-2 border-primary/35 bg-white px-4 font-card text-lg font-black tracking-tight text-slate-800 transition hover:bg-slate-100"
              >
                <RefreshCw className="h-4 w-4" />
                Refresh
              </button>
            </div>

            <div className="mt-3 flex gap-2">
              <input
                value={searchText}
                onChange={(event) => setSearchText(event.target.value)}
                placeholder="Search quizzes..."
                className="h-11 flex-1 rounded-xl border-2 border-primary/35 bg-white/95 px-3 text-base font-semibold text-slate-800 outline-none"
              />
              <button
                onClick={() => setSearchText("")}
                className="h-11 rounded-xl border-2 border-primary/35 bg-white px-4 font-card text-lg font-black tracking-tight text-slate-800 transition hover:bg-slate-100"
              >
                Clear
              </button>
            </div>

            <p className="mt-2 text-sm font-semibold text-slate-600">
              Showing {filteredQuizzes.length} of {quizzes.length}
            </p>

            <div className="mt-3 max-h-[60vh] space-y-3 overflow-auto pr-1">
              {loadingQuizzes && (
                <p className="rounded-xl border border-primary/30 bg-white/70 px-3 py-3 text-sm font-semibold text-slate-700">
                  Loading quizzes...
                </p>
              )}
              {!loadingQuizzes && filteredQuizzes.length === 0 && (
                <p className="rounded-xl border border-primary/30 bg-white/70 px-3 py-3 text-sm font-semibold text-slate-700">
                  No quizzes yet. Create your first quiz.
                </p>
              )}

              {!loadingQuizzes &&
                filteredQuizzes.map((quiz) => (
                  <button
                    key={quiz.id}
                    onClick={() => openQuiz(quiz.id)}
                    className="w-full rounded-xl border-2 border-primary/30 bg-white/85 p-3 text-left transition hover:bg-white"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="line-clamp-2 font-card text-2xl font-black tracking-tight text-slate-900">
                        {quiz.title || "Untitled Quiz"}
                      </h3>
                      <span className="rounded-full border border-primary/30 bg-surface/40 px-2 py-0.5 text-xs font-black uppercase tracking-wide text-slate-700">
                        {formatQuizType(quiz.type)}
                      </span>
                    </div>
                    <p className="mt-1 line-clamp-2 text-sm font-semibold text-slate-700">
                      {quiz.prompt}
                    </p>
                    <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold text-slate-600">
                      <span className="rounded-full border border-primary/25 bg-white px-2 py-0.5">
                        {quiz.questionsCount || 0} questions
                      </span>
                      <span className="rounded-full border border-primary/25 bg-white px-2 py-0.5">
                        {formatDate(quiz.createdAt)}
                      </span>
                    </div>
                  </button>
                ))}
            </div>
          </div>
        </section>

        {error && (
          <p className="mt-5 rounded-xl border border-red-300 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">
            {error}
          </p>
        )}
      </div>
    </div>
  );
}
